---
title: "LLM Fine-Tuning: Phi-3 on Claim Classification"
description: "Fine-tuning Microsoft Phi-3-mini-4k-instruct with LoRA/QLoRA on the LIAR dataset for binary claim classification — +19.5pp accuracy over a zero-shot baseline. Full training pipeline: data preparation, LoRA adapter training, evaluation, and inference."
date: "2025-09-01"
...
Large language models generalize well but are often poorly calibrated for structured classification tasks in specific domains. This project fine-tunes [Microsoft Phi-3-mini-4k-instruct](https://huggingface.co/microsoft/Phi-3-mini-4k-instruct) on the [LIAR dataset](https://huggingface.co/datasets/liar) for binary claim classification (`TRUE` / `FALSE`) using LoRA adapters via Hugging Face PEFT.

The goal was to validate a lightweight methodology for adapting open-weight LLMs in compute-constrained environments, using the same adapter pattern later applied in an internal project on automated screening of regulatory product claims.

This is supervised claim classification rather than end-to-end fact verification: the model predicts LIAR labels from benchmark examples without retrieval or external evidence lookup.

---

## Results

Evaluated on the LIAR test set after collapsing the original six labels into a binary task (`FALSE` = pants-fire / false / barely-true, `TRUE` = half-true / mostly-true / true):
