...
image: ../../assets/images/covers/stats-theory-frequentist.svg
---
...
If you're unsure whether your data is "normal enough" for a parametric test: n ≥ 30 per group is a useful rule of thumb in many applied settings, not a law of nature. Below that, or when the data are clearly skewed or heavy-tailed, check with a quantile-quantile (QQ) plot (`qqnorm()` + `qqline()`) and use a non-parametric alternative if in doubt. The non-parametric versions lose some power but make fewer assumptions.
...
The key assumption that's often violated: logistic regression needs enough outcome events relative to model complexity. The old "10 events per predictor" rule is a rough heuristic, not a guarantee. If you have 50 cases of a disease and 8 predictors, you're already in territory where simpler models or more careful validation become important.
...
The area under the ROC curve (AUC) summarises classifier performance in a single number. AUC = 0.5 is a random classifier. AUC = 1.0 is a perfect classifier. As a rough interpretation guide:
