...
description: "How to use devcontainers and a pre-built personal image to give every team member — including non-developers — an identical, low-setup R + Python environment."
image: ../../assets/images/covers/devcontainers-reproducible-environments.svg
---
...
DevContainers solve this layer. Once the editor integration and container runtime are installed, every team member can open the project and get the same R version, the same tools, and the same VS Code configuration with very little project-specific setup.
...
## Making it work for people who are not primarily developers
...
2. Install Docker Desktop or another compatible Docker runtime.
...
After that initial machine setup, the project-specific friction drops sharply. No `install.packages()`, no Python virtualenv management, no Quarto CLI version mismatch, no TinyTeX setup guide. The environment is effectively the same for everyone on the team.

For a team of biostatisticians or computational biologists who are comfortable with R but not with devops tooling, this is a significant reduction in onboarding friction. The alternative is usually a long "Environment Setup" document that still leaves enough room for drift and local quirks.
