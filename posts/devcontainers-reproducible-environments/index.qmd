...
- **Devcontainer features** — composable installation units. Each feature is a self-contained script that installs a specific tool or set of tools into the container. Features can be published to GHCR or kept as local scripts.
...
2. Configure a CI workflow that builds and pushes the image to GHCR on changes to `.devcontainer/**`
...
- **GHA layer cache** (`cache-from/to: type=gha`) cuts rebuild time significantly — typically from 15+ minutes to 2–3 minutes on incremental changes.
...
- **R packages** — installed via `pak`, supports both CRAN names and GitHub refs (`user/repo@tag`).
...
install_r_deps() {
  local deps=$1
  # Convert comma-separated list to a quoted R vector: a,b,c → "a","b","c"
  local r_vec
