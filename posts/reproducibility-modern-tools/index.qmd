...
image: ../../assets/images/covers/reproducibility-modern-tools.svg
---
...
3. Recreate `.venv` from the lockfile — ensures every collaborator starts from the same declared Python environment.
...
- **Fast in practice** — often noticeably faster than a plain pip workflow, especially with warm caches
...
Reproducibility is no longer optional for serious data work, but it is also not all-or-nothing. The right stack depends on who needs to reproduce the work, how long it needs to stay runnable, and what happens if the result is challenged.

Start with the lowest layer that removes real risk: lock packages with `uv`, `rv`, or `renv`; pin language versions with `rig` or `uv`; add devcontainers when system dependencies start to matter; and document the workflow with Quarto. The goal is not maximum ceremony. It is enough traceability and enough automation for the stakes of the project.
